...
const visibleNavItems = computed(() => {
  if (authStore.isSuperAdmin) {
    return [...primaryAdminNavItems, ...managementAdminNavItems]
  }

  const allowedPaths = new Set(allowedPagePaths.value ?? [])
  const isVisible = (item: AdminNavItem): boolean => allowedPaths.has(item.to)

  return [...primaryAdminNavItems.filter(isVisible), ...managementAdminNavItems.filter(isVisible)]
})
